...
import json
import os
from transformers import AutoProcessor, Gemma3ForConditionalGeneration, FineGrainedFP8Config, AutoModelForCausalLM, AutoTokenizer
from PIL import Image
import requests
import torch
from app.Model import Model
...
class Gemma(Model):
    def __init__(self):
        super().__init__(model_id = "google/gemma-3-12b-it",
    ...
    def load_model(self):
        ...
        self.tokenizer = AutoProcessor.from_pretrained(self.model_id, use_fast=True)
        return True
    ...
    def get_prediction(self, string: str, user_id="default"):
        try:
            ...
            full_prompt = self.tokenizer(full_prompt, return_tensors="pt").to(self.model.device, dtype=torch.bfloat16)
